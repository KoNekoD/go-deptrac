...
func (f *XMLOutputFormatter) Finish(result results2.OutputResult, output pkg.OutputInterface, input OutputFormatterInput) error {
	...
	output.WriteLineFormatted(pkg.StringOrArrayOfStrings{String: fmt.Sprintf("<info>XML Report dumped to %s</>", filepath.Clean(*dumpXmlPath))})
	return nil
}
