...
func (b *BaselineOutputFormatter) Finish(outputResult *results2.OutputResult, output pkg.OutputInterface, outputFormatterInput *OutputFormatterInput) error {
	...
	if stat, _ := os.Stat(dirname); stat == nil || !stat.IsDir() {
		if err := os.MkdirAll(dirname, 0777); err != nil {
			if stat2, _ := os.Stat(dirname); stat2 == nil || !stat2.IsDir() {
				output.WriteLineFormatted(pkg.StringOrArrayOfStrings{String: color.Sprintf("<error>Unable to create %s</>", dirname)})
				return err
			}
		}
	}
	...
	output.WriteLineFormatted(pkg.StringOrArrayOfStrings{String: color.Sprintf("<info>Baseline dumped to %s</>", baselineFile)})
