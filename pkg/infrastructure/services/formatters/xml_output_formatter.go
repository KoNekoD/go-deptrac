...
func (f *XMLOutputFormatter) Finish(result results.OutputResult, output services2.OutputInterface, input OutputFormatterInput) error {
	...
	output.WriteLineFormatted(services2.StringOrArrayOfStrings{String: fmt.Sprintf("<info>XML Report dumped to %s</>", filepath.Clean(*dumpXmlPath))})
	return nil
}
