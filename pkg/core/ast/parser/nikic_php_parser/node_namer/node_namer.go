...
func (n *NodeNamer) GetRootPackageName(path string) (string, error) {
	...
	path, err := util.GetPathWithoutFilename(fullPath)
	if err != nil {
		return "", err
	}
...
func (n *NodeNamer) GetPackageName(path string) (string, error) {
	...
	innerPackageName, err := util.GetPathWithoutFilename(filepath.Join(n.projectDirectory, path))
	if err != nil {
		return "", err
	}
...
func (n *NodeNamer) GetPackageFilename(path string) (string, error) {
	...
	fileName, err := util.GetPathWithOnlyFilename(filepath.Join(n.projectDirectory, path))
	if err != nil {
		return "", err
	}
...
func (n *NodeNamer) GetPackageStructName(path string, structName string) (string, error) {
	...
	pathValidate := strings.Replace(path, "github.com/KoNekoD/go-deptrac/", "/home/mizuki/Documents/dev/KoNekoD/go-deptrac/", 1)
