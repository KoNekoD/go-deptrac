...
func NewFileInputCollector(originalPaths []string, excludedFilePatterns []string, basePath string) (InputCollectorInterface, error) {
	...
	if !fileInfo.IsDir() || !utils.IsReadable(basePath) {
		return nil, apperrors2.NewInvalidPathExceptionUnreadablePath(fileInfo)
	}
	paths := make([]string, 0)
	for _, originalPath := range originalPaths {
		...
		if !utils.IsReadable(path) {
			...
			return nil, apperrors2.NewInvalidPathExceptionUnreadablePath(pathFileInfo)
		}
...
func (c *FileInputCollector) Collect() ([]string, error) {
	if len(c.paths) == 0 {
		return nil, apperrors2.NewException("No 'paths' defined in the depfile.")
	}
	...
	for _, path := range c.paths {
		...
		if err != nil {
			return nil, apperrors2.NewInputExceptionCouldNotCollectFiles(err)
		}
	}
	return paths, nil
}
