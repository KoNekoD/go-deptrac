...
func NewGraphVizOutputImageFormatter(config configuration.FormatterConfiguration) *GraphVizOutputImageFormatter {
...
func (f *GraphVizOutputImageFormatter) output(g *graphviz.Graphviz, graph *cgraph.Graph, output output_formatter_contract2.OutputInterface, input output_formatter_contract2.OutputFormatterInput) error {
	...
	output.WriteLineFormatted(output_formatter_contract2.StringOrArrayOfStrings{String: fmt.Sprintf("<info>Image dumped to %s</>", dumpImagePath)})
	return nil
}
