...
func (b *BaselineOutputFormatter) Finish(outputResult *output_result.OutputResult, output output_formatter_contract2.OutputInterface, outputFormatterInput *output_formatter_contract2.OutputFormatterInput) error {
	...
	if stat, _ := os.Stat(dirname); stat == nil || !stat.IsDir() {
		if err := os.MkdirAll(dirname, 0777); err != nil {
			if stat2, _ := os.Stat(dirname); stat2 == nil || !stat2.IsDir() {
				output.WriteLineFormatted(output_formatter_contract2.StringOrArrayOfStrings{String: color.Sprintf("<error>Unable to create %s</>", dirname)})
				return err
			}
		}
	}
	...
	output.WriteLineFormatted(output_formatter_contract2.StringOrArrayOfStrings{String: color.Sprintf("<info>Baseline dumped to %s</>", baselineFile)})
...
func (b *BaselineOutputFormatter) collectViolations(outputResult *output_result.OutputResult) map[string][]string {
	...
	for _, rule := range append(outputResult.AllOf(result_contract.TypeViolation), outputResult.AllOf(result_contract.TypeSkippedViolation)...) {
