...
func (f *JUnitOutputFormatter) Finish(result output_result.OutputResult, output output_formatter_contract2.OutputInterface, input output_formatter_contract2.OutputFormatterInput) error {
	...
	output.WriteLineFormatted(output_formatter_contract2.StringOrArrayOfStrings{String: fmt.Sprintf("<info>JUnit Report dumped to %s</>", filepath.Clean(*dumpXmlPath))})
	return nil
}
...
func (f *JUnitOutputFormatter) createXML(outputResult output_result.OutputResult) (string, error) {
	...
	for layer, rules := range layers {
		...
		for _, rule := range rules {
			...
			switch r := rule.(type) {
			case *result_contract.Violation:
				testSuite.Failures++
			...
			case *result_contract.SkippedViolation:
				testSuite.Skipped++
				testCase.Skipped = append(testCase.Skipped, Skipped{})
			case *result_contract.Uncovered:
...
func (f *JUnitOutputFormatter) groupRulesByLayer(outputResult output_result.OutputResult) map[string][]result_contract.RuleInterface {
	layers := make(map[string][]result_contract.RuleInterface)
	for _, rule := range outputResult.AllRules() {
		switch r := rule.(type) {
		case result_contract.CoveredRuleInterface:
