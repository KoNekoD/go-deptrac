...
type GraphVizOutputFormatter struct {
	config configuration2.ConfigurationGraphViz
}

func NewGraphVizOutputFormatter(config configuration2.FormatterConfiguration) *GraphVizOutputFormatter {
	extractedConfig := config.GetConfigFor("graphviz").(interface{}).(configuration2.ConfigurationGraphViz)
	return &GraphVizOutputFormatter{config: extractedConfig}
}

func (f *GraphVizOutputFormatter) Finish(result output_result.OutputResult, output output_formatter_contract2.OutputInterface, input output_formatter_contract2.OutputFormatterInput) error {
...
func (f *GraphVizOutputFormatter) calculateViolations(violations []*result_contract2.Violation) map[string]map[string]int {
...
func (f *GraphVizOutputFormatter) calculateLayerDependencies(rules []result_contract2.RuleInterface) map[string]map[string]int {
	layersDependOnLayers := make(map[string]map[string]int)
	for _, rule := range rules {
		switch r := rule.(type) {
		case result_contract2.CoveredRuleInterface:
		...
		case *result_contract2.Uncovered:
...
func (f *GraphVizOutputFormatter) createNodes(outputConfig configuration2.ConfigurationGraphViz, layersDependOnLayers map[string]map[string]int, graph *cgraph.Graph) map[string]*cgraph.Node {
...
func (f *GraphVizOutputFormatter) connectEdges(graph *cgraph.Graph, nodes map[string]*cgraph.Node, outputConfig configuration2.ConfigurationGraphViz, layersDependOnLayers, layerViolations map[string]map[string]int) {
	for layer, layersDependOn := range layersDependOnLayers {
		if slices.Contains(outputConfig.HiddenLayers, layer) {
			continue
		}
...
func (f *GraphVizOutputFormatter) addNodesToGraph(graph *cgraph.Graph, nodes map[string]*cgraph.Node, outputConfig configuration2.ConfigurationGraphViz) {
...
func (f *GraphVizOutputFormatter) output(g *graphviz.Graphviz, graph *cgraph.Graph, output output_formatter_contract2.OutputInterface, input output_formatter_contract2.OutputFormatterInput) error {
	...
	if input.OutputPath != nil && *input.OutputPath != "" {
		...
		output.WriteLineFormatted(output_formatter_contract2.StringOrArrayOfStrings{String: fmt.Sprintf("<info>GraphViz Report saved to %s</>", filepath.Clean(*input.OutputPath))})
		return nil
	}

	output.WriteLineFormatted(output_formatter_contract2.StringOrArrayOfStrings{String: fmt.Sprintf("<info>GraphViz temp image created at %s</>", filename)})
	return nil
}
