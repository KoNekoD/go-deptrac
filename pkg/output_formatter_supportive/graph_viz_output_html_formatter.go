...
func (f *GraphVizOutputHtmlFormatter) output(g *graphviz.Graphviz, graph *cgraph.Graph, output output_formatter_contract2.OutputInterface, input output_formatter_contract2.OutputFormatterInput) error {
	...
	output.WriteLineFormatted(output_formatter_contract2.StringOrArrayOfStrings{String: fmt.Sprintf("<info>HTML dumped to %s</>", filepath.Clean(*dumpHtmlPath))})
	return nil
}
