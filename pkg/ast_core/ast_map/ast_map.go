...
func (a *AstMap) addClassLike(astStructReference ClassLikeReference) {
	...
	// github.com/KoNekoD/go-deptrac/pkg/core/ast_contract/ast_map/emitter.go AstMap
	if strings.Contains(token.ToString(), "github.com/KoNekoD/go-deptrac/pkg/core/ast_contract/ast_map/emitter.go AstMap") {
		panic(token.ToString())
	}
