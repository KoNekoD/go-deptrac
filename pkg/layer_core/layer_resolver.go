...
type LayerResolver struct {
	collectorResolver collector2.CollectorResolverInterface
	layersConfig      []*config_contract.Layer
	layers            map[string][]*collector2.Collectable
	initialized       bool
...
func NewLayerResolver(collectorResolver collector2.CollectorResolverInterface, layersConfig []*config_contract.Layer) layer_resolver_interface.LayerResolverInterface {
	return &LayerResolver{
		collectorResolver: collectorResolver,
		layersConfig:      layersConfig,
		layers:            make(map[string][]*collector2.Collectable),
		resolved:          make(map[string]map[string]bool),
	}
}

// GetLayersForReference retrieves layers for a given reference
func (r *LayerResolver) GetLayersForReference(reference ast_contract.TokenReferenceInterface) (map[string]bool, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *LayerResolver) IsReferenceInLayer(layer string, reference ast_contract.TokenReferenceInterface) (bool, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *LayerResolver) initializeLayers() error {
	r.layers = make(map[string][]*collector2.Collectable)
	for _, layer := range r.layersConfig {
		...
		r.layers[layerName] = []*collector2.Collectable{}
