...
func (c *DirectoryCollector) Satisfy(config map[string]interface{}, reference ast_contract.TokenReferenceInterface) (bool, error) {
...
func (c *DirectoryCollector) GetPattern(config map[string]interface{}) (string, error) {
	if _, ok := config["value"]; !ok {
		if _, ok2 := config["value"].(string); !ok2 {
			return "", layer_contract.NewInvalidCollectorDefinitionExceptionInvalidCollectorConfiguration("DirectoryCollector needs the regex configuration")
		}
	}
