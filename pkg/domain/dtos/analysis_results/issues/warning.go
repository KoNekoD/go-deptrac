...
func NewWarningTokenIsInMoreThanOneLayer(tokenName string, layerNames []string) *Warning {
	return &Warning{
		Message: fmt.Sprintf("%s is in more than one layer_contract [%s]. It is recommended that one token should only be in one layer_contract.", tokenName, strings.Join(layerNames, ", ")),
	}
}
