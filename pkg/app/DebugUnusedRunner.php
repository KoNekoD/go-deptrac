            ...
            foreach ($dependentLayerNames as $dependentLayerName => $numberOfDependencies) {
                if ($numberOfDependencies <= $limit) {
                    if (0 === $numberOfDependencies) {
                        $rows[] = ["<info>{$dependerLayerName}</> layer is not dependent on <info>{$dependentLayerName}</>"];
                    } else {
                        $rows[] = ["<info>{$dependerLayerName}</> layer is dependent <info>{$dependentLayerName}</> layer {$numberOfDependencies} times"];
                    }
                }
            }
        }
