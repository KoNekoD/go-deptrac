...
func (f *XMLOutputFormatter) Finish(result output_result.OutputResult, output output_formatter.OutputInterface, input output_formatter.OutputFormatterInput) error {
	...
	output.WriteLineFormatted(output_formatter.StringOrArrayOfStrings{String: fmt.Sprintf("<info>XML Report dumped to %s</>", filepath.Clean(*dumpXmlPath))})
	return nil
}
