...
func (t *TableOutputFormatter) GetName() output_formatter.OutputFormatterType {
	return output_formatter.Table
}

func (t *TableOutputFormatter) Finish(outputResult *output_result.OutputResult, output output_formatter.OutputInterface, outputFormatterInput *output_formatter.OutputFormatterInput) error {
	groupedRules := make(map[string][]result.RuleInterface)
	...
	for _, layer := range groupedRulesLayers {
		...
		slices.SortFunc(rules, func(a, b result.RuleInterface) int {
...
func (t *TableOutputFormatter) skippedViolationRow(rule *result.SkippedViolation) []string {
...
func (t *TableOutputFormatter) printSummary(result *output_result.OutputResult, output output_formatter.OutputInterface, reportUncoveredAsError bool) {
	...
	style.DefinitionList(
		[]output_formatter.StringOrArrayOfStringsOrTableSeparator{
			{String: "Report"},
			{TableSeparator: true},
...
func (t *TableOutputFormatter) uncoveredRow(rule *result.Uncovered, reportAsError bool) []string {
	gotDependency := rule.GetDependency()
...
func (t *TableOutputFormatter) printErrors(result *output_result.OutputResult, output output_formatter.OutputInterface) {
...
func (t *TableOutputFormatter) printWarnings(result *output_result.OutputResult, output output_formatter.OutputInterface) {
