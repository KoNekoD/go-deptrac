...
func (f *GraphVizOutputImageFormatter) output(g *graphviz.Graphviz, graph *cgraph.Graph, output output_formatter.OutputInterface, input output_formatter.OutputFormatterInput) error {
	...
	output.WriteLineFormatted(output_formatter.StringOrArrayOfStrings{String: fmt.Sprintf("<info>Image dumped to %s</>", dumpImagePath)})
	return nil
}
