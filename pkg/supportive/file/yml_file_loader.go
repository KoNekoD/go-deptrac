...
import (
	"github.com/KoNekoD/go-deptrac/pkg/supportive/file/exception"
	"gopkg.in/yaml.v3"
)
...
func (y *YmlFileLoader) ParseFile(file string) (*ParseFileResult, error) {
	...
	if err != nil {
		return nil, exception.NewFileCannotBeParsedAsYamlExceptionFromFilenameAndException(file, err)
	}
	...
	if !ok1 || !ok2 || !ok3 {
		return nil, exception.NewParsedYamlIsNotAnArrayExceptionFromFilename(file)
	}
