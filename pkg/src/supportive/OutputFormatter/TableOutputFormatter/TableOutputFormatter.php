...
final class TableOutputFormatter implements OutputFormatterInterface
{
    ...
    private function skippedViolationRow(SkippedViolation $rule) : array
    {
        ...
        $message = \sprintf('<info>%s</> must not depend on <info>%s</> (%s)', $dependency->getDepender()->toString(), $dependency->getDependent()->toString(), $rule->getDependentLayer());
    ...
    {
        $dependency = $rule->getDependency();
        $message = \sprintf('<info>%s</> must not depend on <info>%s</>', $dependency->getDepender()->toString(), $dependency->getDependent()->toString());
    ...
    {
        $dependency = $rule->getDependency();
        $message = \sprintf('<info>%s</> has uncovered dependency on <info>%s</>', $dependency->getDepender()->toString(), $dependency->getDependent()->toString());
        if (count($dependency->serialize()) > 1) {
            $message .= "\n" . $this->formatMultilinePath($dependency);
        }
        $fileOccurrence = $rule->getDependency()->getContext()->fileOccurrence;
