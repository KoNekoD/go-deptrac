...
final class ConsoleOutputFormatter implements OutputFormatterInterface
{
    ...
    private function printViolation(Violation|SkippedViolation $rule, OutputInterface $output) : void
    {
        ...
        $output->writeLineFormatted(\sprintf('%s<info>%s</> must not depend on <info>%s</> (%s on %s)', $rule instanceof SkippedViolation ? '[SKIPPED] ' : '', $dependency->getDepender()->toString(), $dependency->getDependent()->toString(), $rule->getDependerLayer(), $rule->getDependentLayer()));
    ...
    private function printSummary(OutputResult $result, OutputInterface $output) : void
    {
        ...
        $output->writeLineFormatted(\sprintf('<info>Allowed: %d</>', $allowedCount));
    ...
    private function printUncovered(OutputResult $result, OutputInterface $output) : void
    {
        ...
        $output->writeLineFormatted('<comment>Uncovered dependencies:</>');
        foreach ($uncovered as $u) {
            $dependency = $u->getDependency();
            $output->writeLineFormatted(\sprintf('<info>%s</> has uncovered dependency on <info>%s</> (%s)', $dependency->getDepender()->toString(), $dependency->getDependent()->toString(), $u->layer));
            $this->printFileOccurrence($output, $dependency->getContext()->fileOccurrence);
            if (count($dependency->serialize()) > 1) {
                $this->printMultilinePath($output, $dependency);
            }
