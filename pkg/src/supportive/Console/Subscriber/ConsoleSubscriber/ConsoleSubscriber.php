...
class ConsoleSubscriber implements EventSubscriberInterface
{
    ...
    public function onPreCreateAstMapEvent(PreCreateAstMapEvent $preCreateAstMapEvent) : void
    {
        if ($this->output->isVerbose()) {
            ...
            $this->output->writeLineFormatted(sprintf('Start to create an AstMap for <info>%u</> Files.', $preCreateAstMapEvent->expectedFileCount));
        }
    }
    public function onPostCreateAstMapEvent(PostCreateAstMapEvent $postCreateAstMapEvent) : void
    {
        if ($this->output->isVerbose()) {
            $this->printMessageWithTime('ast', '<info>AstMap created in %01.2f sec.</>', '<info>AstMap created.</>');
        }
    }
    ...
    public function onPreDependencyEmit(PreEmitEvent $event) : void
    {
        if ($this->output->isVerbose()) {
            ...
            $this->output->writeLineFormatted(sprintf('start emitting dependencies <info>"%s"</>', $event->emitterName));
        }
    }
    public function onPostDependencyEmit(PostEmitEvent $event) : void
    {
        if ($this->output->isVerbose()) {
            $this->printMessageWithTime('deps', '<info>Dependencies emitted in %01.f sec.</>', '<info>Dependencies emitted.</>');
        }
    }
    ...
    {
        if ($this->output->isVerbose()) {
            $this->printMessageWithTime('flatten', '<info>Dependencies flattened in %01.f sec.</>', '<info>Dependencies flattened.</>');
        }
    }
    /**
