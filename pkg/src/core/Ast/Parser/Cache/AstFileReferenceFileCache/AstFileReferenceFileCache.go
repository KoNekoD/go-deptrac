package AstFileReferenceFileCache
...
type AstFileReferenceFileCache struct {
	cache map[string]struct {
		hash      string
		reference *AstMap.FileReference
	}
	loaded       bool
...
func (c *AstFileReferenceFileCache) Get(filepath string) (*AstMap.FileReference, error) {
	err := c.Load()
	if err != nil {
		return nil, err
	}
...
func (c *AstFileReferenceFileCache) Set(fileReference *AstMap.FileReference) error {
	err := c.Load()
	if err != nil {
		return err
	}
