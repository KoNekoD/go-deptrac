...
type FileReferenceVisitor struct {
	dependencyResolvers  []ReferenceExtractorInterface.ReferenceExtractorInterface
	currentTypeScope     *TypeScope.TypeScope
	currentReference     AstMap.ReferenceBuilderInterface
	fileReferenceBuilder *AstMap.FileReferenceBuilder
	typeResolver         *TypeResolver.TypeResolver
	nestingStack         []ast.Node
}

func NewFileReferenceVisitor(fileReferenceBuilder *AstMap.FileReferenceBuilder, resolver *TypeResolver.TypeResolver, extractors ...ReferenceExtractorInterface.ReferenceExtractorInterface) *FileReferenceVisitor {
	return &FileReferenceVisitor{
		...
		currentTypeScope:     TypeScope.NewTypeScope(""),
	}
}
...
func (f *FileReferenceVisitor) enterNode(node ast.Node) {
	...
	case *ast.File:
		f.currentTypeScope = TypeScope.NewTypeScope(typedNode.Name.Name).SetFileNode(typedNode)
	case *ast.FuncDecl:
